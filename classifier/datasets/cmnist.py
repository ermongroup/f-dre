...
class CMNIST(VisionDataset):
    '''
    MNIST with alpha% of samples recolored with blue background and (100-alpha)% yellow.
    '''
    def __init__(self,
                args, 
                split='train',
        ...
        mnist = MNIST()
        #datasets.MNIST(
            # self.root, train=True if self.split == 'train' else False, 
            # download=True)
        self.og_data = mnist.data
        self.og_y = mnist.targets
    ...
    def initialize_data_splits(self):
        ...
        dset.append(self.og_data[idx])
        labels.append(torch.zeros(len(idx)))

        # y = 1
        idx = torch.where(self.og_y >= 5)[0]
        dset.append(self.og_data[idx])
        labels.append(torch.ones(len(idx)))
