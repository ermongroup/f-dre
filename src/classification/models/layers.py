"""
https://github.com/ermongroup/ncsnv2/blob/master/models/layers.py
"""
import math
import string
...
from src.classification.models.normalizers import *
...
def dilated_conv3x3(in_planes, out_planes, dilation=1, stride=1, bias=True, spec_norm=False):
...
def contract_inner(x, y):
    ...
    stop = len(x.shape) - 1
    y_chars = list(string.ascii_lowercase[stop:stop+len(y.shape)])
...
class NIN(nn.Module):
    def __init__(self, in_dim, num_units, init_scale=1.):
        ...
        self.W = nn.Parameter(torch.randn(self.in_dim, self.num_units) * self.init_scale)
        self.b = nn.Parameter(torch.zeros(self.num_units, ))
