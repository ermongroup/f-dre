...
class TREResnetClassifier(nn.Module):
    def __init__(self, args, config):
        ...
        self.fc_list = nn.ModuleList(
            nn.Linear(784, 2) for _ in range(self.config.tre.m)
        )
        # self.fc = nn.Linear(784, 2)
        # self.fc = nn.Linear(3072, 1)
    ...
    def forward(self, x):
        # collapse all bridges together for shared network
        h = x.view(-1, self.channels, 28, 28)
        output = self.begin_conv(h)
        ...
        # classification
        outputs = output.view(self.m, int(len(x) / self.m), -1)
        logits = []
        for x, fc in zip(outputs, self.fc_list):
            logits.append(fc(x))
